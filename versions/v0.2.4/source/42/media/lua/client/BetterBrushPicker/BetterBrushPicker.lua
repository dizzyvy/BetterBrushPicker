...
-- Author: Dizzy
--
-- v0.2.4 replaces the old fixed-slot probing scan with direct enumeration of the tiles exposed by IsoWorld:getAllTiles. Persistent per-tileset caching is kept.
...
-- Build a direct lookup of the actual tile IDs known to IsoWorld.
-- getAllTiles() is documented as HashMap<String, ArrayList<String>>, while
-- getAllTiles(String filename) exposes the list for one tileset/definition.
-- We keep both routes so a naming difference between tile-image names and
-- definition filenames does not silently remove tiles from the index.
local function buildDirectTileLookup(tilesets)
    local world = getWorld()
    local lookup = {}
    local sourceMap = nil
    ...
    if mapOk then sourceMap = mapResult end

    if sourceMap ~= nil then
        local keysOk, keys = pcall(function() return sourceMap:keySet() end)
        if keysOk and keys ~= nil then
            local iteratorOk, iterator = pcall(function() return keys:iterator() end)
            if iteratorOk and iterator ~= nil then
                while true do
                    ...
                    local nextOk, key = pcall(function() return iterator:next() end)
                    if not nextOk then break end
                    local keyString = tostring(key)
                    local base = keyString:match("^([^%.]+)") or keyString
                    local valueOk, value = pcall(function() return sourceMap:get(key) end)
                    if valueOk and value ~= nil then
                        lookup[base] = value
                        lookup[keyString] = value
                    end
                end
            end
        end
    ...
    for i = 1, #tilesets do
        ...
        if tileList == nil then
            -- Some definitions may retain an extension in the Java map key.
            for suffix in {".tiles", ".tileset", ".png", ".txt"} do
                local candidate = lookup[tileset .. suffix]
        ...
        if tileList ~= nil then
            ...
            if sizeOk and size ~= nil then count = size end
        end

        table.insert(queue, { tileset = tileset, tiles = tileList, count = count })
        totalDirectIds = totalDirectIds + count
        if count == 0 then zeroCount = zeroCount + 1 end
    end

    print(string.format("[BetterBrushPicker] Direct tile enumeration found %d tile IDs across %d tilesets (%d empty sets).", totalDirectIds, #queue, zeroCount))
    return queue
end
...
local function loadExistingCacheSets(tilesetQueue)
    local loaded = {}
    local missing = {}
    local cachedSets = 0
...
function BetterBrushPicker.startIndexBuild()
    ...
    local tileQueue = buildDirectTileLookup(allTilesets)
    local directTotal = 0
    for i = 1, #tileQueue do
        directTotal = directTotal + (tileQueue[i].count or 0)
    end
    BetterBrushPicker.index = {}
    BetterBrushPicker.tilesets = {}

    loadExistingCacheSets(tileQueue)

    if directTotal == 0 and #allTilesets > 0 then
        BetterBrushPicker.indexBuilding = false
        BetterBrushPicker.indexReady = false
        BetterBrushPicker.indexState.error = "B42 returned no tile IDs through IsoWorld:getAllTiles()"
        print("[BetterBrushPicker] ERROR: direct tile enumeration returned zero IDs. Scan stopped; no empty caches were intentionally generated.")
        return
    end
    ...
    local zeroCount = BetterBrushPicker.indexState.missingTilesets or 0
    if zeroCount > 0 then
        print(string.format("[BetterBrushPicker] WARNING: %d tilesets returned no direct tile list; those sets will be cached empty unless B42 exposes them under a different definition name.", zeroCount))
    end
end
...
function BetterBrushPicker.tickIndexBuild()
    ...
    while budget > 0 and state.tilesetIndex <= #BetterBrushPicker.tilesets do
        ...
        if ok and tileName ~= nil and tileName ~= "nil" and tileName ~= "" then
...
local function install()
    ...
    print("[BetterBrushPicker] Installed v0.2.4 successfully.")
    return true
end
