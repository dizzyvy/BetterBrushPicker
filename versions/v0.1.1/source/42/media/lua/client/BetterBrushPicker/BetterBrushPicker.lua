...
-- B42 can load the debug Brush Tool classes after OnGameBoot.
-- Retry briefly until the vanilla picker classes exist, then install once.
local function waitForVanillaBrushTool()
    if BetterBrushPicker.installed then
        Events.OnTick.Remove(waitForVanillaBrushTool)
        return
    end

    if BrushToolChooseTileUI ~= nil and BrushToolTilePickerList ~= nil and ISBrushToolTileCursor ~= nil then
        install()
        if BetterBrushPicker.installed then
            Events.OnTick.Remove(waitForVanillaBrushTool)
        end
...
Events.OnGameBoot.Add(function()
    -- Try immediately. If the debug UI scripts are not loaded yet, OnTick will catch them.
    install()
    if not BetterBrushPicker.installed then
        Events.OnTick.Add(waitForVanillaBrushTool)
    end
end)
