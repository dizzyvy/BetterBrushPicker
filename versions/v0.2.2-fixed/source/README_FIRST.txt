...
V0.2.2 SAFE INDEXING
- The full tile catalog is still scanned.
- Scanning happens in small bursts instead of one large blocking pass.
- Each completed tileset is written to a separate persistent cache file in the Zomboid user data folder.
- If the game crashes or closes during indexing, completed tilesets remain cached and the next launch resumes the missing tilesets.
- Cached tilesets are loaded on later launches without rescanning them.
- Tile existence checks use getSprite() during indexing; textures are only requested when results are displayed.
- Cache files use a version header so this cache format can be invalidated safely by a future release.
